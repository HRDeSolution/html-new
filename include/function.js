...
/*
if(location.hostname == "manager.hrde.co.kr"){
	location.href = "https://manager.hrde.co.kr/hrd_manager/";
}
*/
...
function LoginSubmit_work() {
	...
	document.LoginForm.MemberType.value = checked_value;
	document.LoginForm.action = '/public/member/login_ok_work.php';
	document.LoginForm.submit();
}
...
function Inquiry(){
	...
	$('#DataResult').load('/archive/main/inquiry_pop.php', { t: '1'}, function () {
...
function StudyProgressCheck(ProgressStep, CloseYN, ContentsURLSelect) {
    ...
    // console.log('Chapter_Seq : ', Chapter_Seq);
    // console.log('Contents_idx : ', Contents_idx);
    // console.log('ContentsDetail_Seq : ', ContentsDetail_Seq);
...
function resizeIframe(fr) {
	...
	if (screenWidth <= 1550) {
        mPlayer_width = "100%";
		mPlayer_height  = (screenWidth * 750 / 1200) + "px";
    }
...
//초단위로 수강시간 보여주는 부분 똑같은 함수가 두개여서 주석처리함
// function StudyTimeCheck() {
// 	var AddTime = parseInt($('#StartTime').val()) + 1;
...
function CertificatePrintMulti(CompanyCode, LectureStart, LectureEnd, LectureCode, CertificatePrintOK, ID) {
...
function CourseLike(data, LectureCode, ID){
	...
	if(ID){
		...
		$.post('/archive/contents/course_like.php', { LectureCode: LectureCode, clickVal: clickVal }, function (data) {
			if (data == 'like') {
...
function ManagerCourseCheck(LectureStart, LectureEnd, LectureCode, ID, Seq) {
	...
	var url = '/public/mypage/manager_trainee_list.php?LectureStart=' + LectureStart + '&LectureEnd=' + LectureEnd + '&LectureCode=' + LectureCode +'&ID=' + ID +'&Seq=' + Seq;
...
function CertificatePrint(LectureCode) {
	...
	$('#DataResult').load('/public/mypage/certificate_layer.php', { LectureCode: LectureCode }, function () {
...
function PlayStudyCounsel(LectureCode, Contents_idx) {
	...
	$('#StudyInformation').load('/player/study_counsel.php', { LectureCode: LectureCode, Contents_idx: Contents_idx }, function () {
...
function archiveReport(CompanyCode, LectureStart, LectureEnd ) {
    var url =
        '/include/archive_report.html?CompanyCode=' + CompanyCode + '&LectureStart='+LectureStart +'&LectureEnd='+LectureEnd;
    window.open(url, 'certi', 'scrollbars=yes, resizable=yes, left=400, width=820, height=700');
}

//수강후기 팝업
function SurveyPop(ContentsURLSelect, LectureCode, ChkData) {
    ScrollPosition = 100;
	left_position = 50;


	$('#StudyInformation').load('/player/survey_pop.php', { ContentsURLSelect: ContentsURLSelect, LectureCode: LectureCode, ChkData: ChkData}, function () {
		$("div[id='StudyInformation']")
			.css({
				top: ScrollPosition,
				left: left_position,
...
function PlayStudyAuth(Chapter_Number, LectureCode, Study_Seq, StudyLectureCode, EvalCd) {
	var currentWidth = $(window).width();
	var LocWidth = currentWidth / 2;
	var body_width = screen.width - 20;
	...
	$('#DataResult').load(
		...
		{ Chapter_Number: Chapter_Number, LectureCode: LectureCode, Study_Seq : Study_Seq, StudyLectureCode : StudyLectureCode, EvalCd : EvalCd},
		function () {
			//$('html, body').animate({ scrollTop : 0 }, 300);
			$("div[id='DataResult']")
				.css({
