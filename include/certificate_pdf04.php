...
$where[] = "e.LectureCode='$LectureCode'";
...
$JoinQuery = " Study AS a 
            ...
            LEFT OUTER JOIN (SELECT DISTINCT LectureCode, ID , Study_Seq FROM Progress) AS e ON a.ID=e.ID AND a.Seq=e.Study_Seq
            LEFT OUTER JOIN Course AS d ON e.LectureCode=d.LectureCode ";

$NotPDF = "N";

$Sql2 = "SELECT COUNT(a.Seq)
...
if($NotPDF=="Y") {
	...
	if($QUERY && mysqli_num_rows($QUERY)){
    	while($ROW = mysqli_fetch_array($QUERY)){
    		...
    		//if($LectureCode == "A0905282") {
    		if(mb_strlen($ContentsName, "UTF-8") > 29) {
    		    $contentsName1 = mb_substr($ContentsName, 0, 29, "UTF-8");
    		    $contentsName2 = mb_substr($ContentsName, 29, null, "UTF-8");
    		    
    		    $pdf->WriteHTML("                훈련과정 : ". iconv("UTF-8", "CP949", $contentsName1));
    		    $pdf->Ln(7);
    		    $pdf->WriteHTML("                                ". iconv("UTF-8", "CP949", $contentsName2));
    		}else {
    		    $pdf->WriteHTML("                훈련과정 : ". iconv("UTF-8", "CP949", $ContentsName));
    		}
