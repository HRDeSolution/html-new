...
class Page
{
	...
	function blockList(){
		...
		$block_str = '<div class="grid-nav swiper-nav">';
		...
		if($this->block != 1){
			...
			$block_str .= '<div class="circleprev circleprev1"><a href="javascript:pageRun(' . $temp . ');" title="이전 ' . $this->block_size . '" ><i class="ph-bold ph-caret-left"></i></a></div>';
		}else{
			$block_str .= '';
		}

		//--블럭 리스트
		$block_str .= '<div class="grid-pagination swiper-pagination">';
		$arrBlock = array();
		...
		for($i = 0; $i < count($arrBlock); $i++){
			if($this->pg != $arrBlock[$i]){
				...
				//$block_str .= '<li class="on"><a href="javascript:pageRun('. $arrBlock[$i] . ');" style="color:black">' . $arrBlock[$i] . '</a></li>';
				$block_str .= '<span class="on"><a href="javascript:pageRun('. $arrBlock[$i] . ');" >' . $arrBlock[$i] . '</a></span>';
			}
		...
		if($this->block != $this->block_count && $this->tot_no != 0){
			...
			$block_str .= '<div class="circlenext circlenext1"><a href="javascript:pageRun(' . $temp . ')" title="다음 ' . $this->block_size . '"><i class="ph-bold ph-caret-right"></i></a></div>';
		}else{
			$block_str .= '';
		}
