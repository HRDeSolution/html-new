...
$CertSiteName2 = "HRDe아카이브";
$CertSiteName3 = "HRDe아카이브";

$UPLOAD_DIR = "/web/hrde/html/upload";
$HomeDirectory = "/web/hrde/html";

$SiteURL 			= $Protocol_SSL."www.hrdeedu.co.kr";
$MobileSiteURL 		= $Protocol_SSL."m.hrdeedu.co.kr";

$MobileAuthURL 		= $Protocol_SSL."hrdeedu.co.kr/"; //모바일 인증 도메인
$FlashServerURL 	= $Protocol_SSL."www.hrdeedu.co.kr/contents";
$MovieServerURL		= $Protocol_SSL."www.hrdeedu.co.kr/contents";
$MobileServerURL 	= $Protocol_SSL."www.hrdeedu.co.kr/contents";
...
$IPIN_CheckPlus_sitecode = "HR62";
$IPIN_CheckPlus_sitepasswd = "W6s8kCvp";


$DB_Enc_Key = "ek3434!";


$captcha_agent_id = 'hrda1746'; //이몬


//문자발송관련필수값
$apikey = "s3qomyek1qd54jdhswhc6g7i051ntfl9";
$aligoUserid = "hrde1746";
$aligoSenderkey = "e4201070f3824bf0364e2f0ec60543384d752ab0";
$TRAN_CALLBACK = "18119530";
...
$Manager_Top_Link_array = array(
    ...
    //수강관리방
    "B1" => "study_ip.php", //IP모니터링
    "B2" => "certification_list.php", //본인인증 모니터링
    
    //독려관리
    "C1" => "study_sms.php", //학습참여독려
    "C2" => "study_sms_log.php", //문자발송내역
    "C3" => "study_email_log_for_edumanager.php", //메일발송내역(교육담당자)
    "C4" => "study_email_log.php", //메일발송내역(훈련생)
    "C5" => "smsdata.php", //문자/템플릿관리
    
    //컨텐츠관리
    "D1" => "course_category.php", //과정카테고리 관리
    "D2" => "contents.php", //기초차시 관리
    "D3" => "ability_question_manager.php", //역량진단 문항관리
    "D4" => "ability_manager.php", //역량진단관리
    "D5" => "satisfaction_manager.php", //만족도 조사
    
    //사업주훈련
    "E1" => "course.php?ctype=X", //이러닝컨텐츠관리
    "E2" => "course.php?ctype=Z", //마이크로닝컨텐츠관리
    "E3" => "course.php?ctype=Y", //숏폼컨텐츠관리
    "E4" => "contents_flex_keyword.php", //컨텐츠키워드관리
    "E5" => "flex_contents.php", //PICK/TOP/NEW 컨텐츠 관리
    "E6" => "flex_like.php", //관심강의관리
    "E7" => "inquiry.php", //수강신청관리
    "E8" => "study.php", //학습관리
    "E9" => "study_end.php", //수강마감
    
    //커뮤니티관리
    "F1" => "notice.php", //공지사항
    "F2" => "faq.php", //자주묻는질문
    "F3" => "qna.php", //1:1상담/학습상담
    "F6" => "cheating_report.php", //부정제보
    
    //사이트관리
    "G1" => "site_info.php", //사이트정보관리
...
$Cheating_array = array(
    "A" => "전화",
    "B" => "이메일",
    "C" => "방문",
    "D" => "기타",
);
reset($Cheating_array);
...
$ContentsKeyword_array = array(
    "1" => "직급",
    "2" => "직무분야",
    "3" => "관심분야",
    "4" => "역량"
);
reset($ContentsKeyword_array);
...
$abilityDepth_array = array(
    "step01" => "1단계",
    "step02" => "2단계",
    "step03" => "3단계"
);
reset($abilityDepth_array);

$abilityGroup_array = array(
    "A" => "-",
    "W" => "직무(실무자)",
    "M" => "직무(관리자)",
    "C" => "공통",
...
reset($abilityGroup_array);

$abilityBind_array = array(
    "A" => "경영의식",
    "B" => "고객지향성",
    "C" => "변화혁신마인드",
    "D" => "목표관리능력",
...
reset($abilityBind_array);

$abilityValue_array = array(
    "1" => "그렇지않다",
    "2" => "그렇지않은편이다",
    "3" => "보통이다",
    "4" => "그런편이다",
...
reset($abilityValue_array);

$abilityType_array = array(
    "B" => "사전",
    "A" => "사후"
);
reset($abilityType_array);
...
// 알리고 카톡발송함수
function aligo_mms_send($phone, $subject, $emtitle, $msg, $callback, $template_code="hrd01") {
    global $connect, $apikey, $aligoUserid, $aligoSenderkey;

    // 에러메시지 초기화
    $GLOBALS['ALIGO_LAST_ERRMSG'] = '';

    if ($phone)  {
        $callback_arr = explode("/",$callback);
        $callback = preg_replace('/\D+/', '', trim($callback_arr[0])); // 숫자만 권장

        /* 1) 토큰 발급 */
        $_apiURL    = 'https://kakaoapi.aligo.in/akv10/token/create/30/s/';
        $_hostInfo  = parse_url($_apiURL);
        $_port      = (strtolower($_hostInfo['scheme']) == 'https') ? 443 : 80;
        $_variables = array('apikey' => $apikey, 'userid' => $aligoUserid);
        ...
        if ($error_msg) {
            $GLOBALS['ALIGO_LAST_ERRMSG'] = 'cURL(token): '.$error_msg;
            return "N2";
        }

        $retArr = json_decode($ret);
        if (!$retArr || !isset($retArr->code) || $retArr->code != 0 || empty($retArr->token)) {
            // 토큰 단계 실패 상세 저장
            $msgText = '';
            if (is_object($retArr) && isset($retArr->message)) {
                $msgText = is_string($retArr->message) ? $retArr->message : json_encode($retArr->message, JSON_UNESCAPED_UNICODE);
            } else {
                $msgText = (string)$ret;
            }
            $GLOBALS['ALIGO_LAST_ERRMSG'] = 'token_fail: '.$msgText;
            return "N2";
        }
        $token = $retArr->token;
        ...
        if ($error_msg) {
            $GLOBALS['ALIGO_LAST_ERRMSG'] = 'cURL(send): '.$error_msg;
            return "N2";
        }

        $retArr = json_decode($ret, true);
        $result_code = isset($retArr['code']) ? (int)$retArr['code'] : -1;
        ...
        if ($result_code === 0) {
            ...
            // 실패 상세
            $detail = $retArr['message'] ?? '';
            if (is_string($detail)) {
                $j = json_decode($detail, true);
                if (json_last_error() === JSON_ERROR_NONE) $detail = print_r($j, true);
            } elseif (is_array($detail)) {
                $detail = print_r($detail, true);
            }
            if ($detail === '') $detail = (string)$ret;
            $GLOBALS['ALIGO_LAST_ERRMSG'] = $detail;
            return "N2";
        }
    } else {
        $GLOBALS['ALIGO_LAST_ERRMSG'] = 'empty phone';
        return "N1";
    }
}
...
// 알리고 문자발송함수
function aligo_send($phone, $msg, $callback) {
    global $connect;
    global $apikey;
    global $aligoUserid;
    global $aligoSenderkey;
    ...
    if ($phone)  {
        ...
        $_POST['sender'] = $TRAN_CALLBACK; // 발신번호
        ...
        if ($retArr->code != 0) {
            $result_code = $retArr->code;
        }
        $report_date = $response->received_at;
        $result_date = $response->received_at;
        
        if ($result_code)   return "Y";
        else if ($result_code == 0) return "Y";
        else    return "N2";
...
function StarPointViewE($StarPoint, $mode='html') {
    $StarPoint = max(0, min(5, (int)$StarPoint));
    if ($mode === 'excel') {
        return str_repeat('★', $StarPoint) . str_repeat('☆', 5 - $StarPoint);
    }
    $on  = "<img src='/images/common/icon_review_star03.png' alt='' />";
    $off = "<img src='/images/common/icon_review_star01.png' alt='' />";
    return str_repeat($on, $StarPoint) . str_repeat($off, 5 - $StarPoint);
}
...
function ParseUserAgent($userAgent) {
    ...
    if (preg_match('/MSIE ([0-9].[0-9]+)/i', $userAgent, $matches)) {
        ...
        if (preg_match('/Version\/([0-9\.]+)/i', $userAgent, $versionMatch)) {
            $browser = 'Safari';
 ...
 * 파일 체크 함수
 */
function checkFileSecure($file, $originalFileName, $onError = null)
{
	$allowed_ext = [
		'hwp', 'hwpx',               
		'xlsx', 'xls',               
	...
	if (!in_array($ext, $allowed_ext)) {
		return handleFileSecureError("허용되지 않는 확장자입니다. ($ext)", $onError);
	}

	if (!in_array($mime_type, $allowed_mime_types)) {
		return handleFileSecureError("허용되지 않는 MIME 타입입니다. ($mime_type)", $onError);
	}
	...
	if ($ext === 'zip') {
		...
		if ($zip->open($file) === TRUE) {
			for ($i = 0; $i < $zip->numFiles; $i++) {
				...
				if ($entryExt && in_array($entryExt, $forbidden_ext)) {
					$zip->close();
					return handleFileSecureError("압축파일에 금지된 파일이 포함되어 있습니다: $entry", $onError);
				}
			}
			$zip->close();
		} else {
			return handleFileSecureError("압축파일을 열 수 없습니다.", $onError);
		}
	}
...
function handleFileSecureError($msg, $onError = null)
{
	if (is_callable($onError)) {
		call_user_func($onError, $msg);
	} else {
		// 기본 종료 처리 (팝업 닫기)
		echo "<script>alert('".addslashes($msg)."'); top.UploadFileClose();</script>";
	}
	exit;
}
?>