...
if($ATypeEA>0 && $BTypeEA==0 && $CTypeEA==0) {
	...
	foreach($ExamA_idx_array as $ExamA_idx_array_value) {
		...
		if($UserAnswer==$Answer) {
			...
			if($i==0)    $UserAPoint_Str = $UserAPoint;
			else         $UserAPoint_Str = $UserAPoint_Str."|".$UserAPoint;

		}else{
			$UserAPoint = 0;

			if($i==0)    $UserAPoint_Str = "0";
			else         $UserAPoint_Str = $UserAPoint_Str."|0";
	...
	$ReMid = $Row['ReMid']; //중간평가 재응시여부
	$ReTest = $Row['ReTest']; //최종평가 재응시여부
	$ReReport = $Row['ReReport']; //과제 재응시여부

	if(!$MidScore)     $MidScore = 0;
	if(!$TestScore)    $TestScore = 0;
	if(!$ReportScore)  $ReportScore = 0;
	...
	if($Progress>=$PassProgress && $TotalScore>=$PassScore) {
		...
		$PassOK = "N";
		if($Progress<$PassProgress)   $FailReason = "진도 부족";
		if($TotalScore<$PassScore) {
		    if($FailReason)  $FailReason = $FailReason.", 평가점수 미달";
		    else  $FailReason = "평가점수 미달";			
		}
	}
