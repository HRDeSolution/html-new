...
if($str=="A") $LectureRegResult = "대기";
if($str=="B") $LectureRegResult = "<font color='blue'>등록</font>";
if($str=="C") $LectureRegResult = "<font color='red'>오류</font>";
?>
...
if($QUERY && mysqli_num_rows($QUERY)){
	while($ROW = mysqli_fetch_array($QUERY)){
		...
		if($LectureTerm_check > 0 || !$LectureStart || !is_numeric($LectureStart_array[0]) || strlen($LectureStart_array[0]) != 4 || !is_numeric($LectureStart_array[1]) || strlen($LectureStart_array[1]) != 2 || !is_numeric($LectureStart_array[2]) || strlen($LectureStart_array[2]) != 2 || checkdate($LectureStart_array[1], $LectureStart_array[2], $LectureStart_array[0])==false) {
		...
		if($LectureTerm_check > 0 || !$LectureEnd || !is_numeric($LectureEnd_array[0]) || strlen($LectureEnd_array[0]) != 4 || !is_numeric($LectureEnd_array[1]) || strlen($LectureEnd_array[1]) != 2 || !is_numeric($LectureEnd_array[2]) || strlen($LectureEnd_array[2]) != 2 || checkdate($LectureEnd_array[1], $LectureEnd_array[2], $LectureEnd_array[0])==false) {
		...
		if(!$ServiceType) {
			...
			if($ServiceType2_array[$ServiceType]=="") {
		...
		if(!$UserID) {
		    ...
		    //241113.YEON
		    //Member테이블에 입력한ID가 있는지확인
		    $query_chk1 = "SELECT COUNT(*) FROM Member WHERE ID='$UserID'";
		    $result_chk1 = mysqli_query($connect, $query_chk1);
		    $row_chk1 = mysqli_fetch_array($result_chk1);
		    //[1]있는경우
		    if($row_chk1[0] > 0){
		        //[1-1]입력한 주민번호와 DB의 주민번호가 일치하는지 확인
		        ...
		        $query_chk2 = "SELECT AES_DECRYPT(UNHEX(ResNo),'$DB_Enc_Key') AS ResNo FROM Member WHERE ResNo=HEX(AES_ENCRYPT('$InputResNo','$DB_Enc_Key'))";
		        $result_chk2 = mysqli_query($connect, $query_chk2);
		        $row_chk2 = mysqli_fetch_array($result_chk2);
		        $DB_ResNo = $row_chk2[0];
		        
		        $ResNo1 = substr($DB_ResNo,0,6);
		        $ResNo2 = substr($DB_ResNo,6,7);
		        $ResNoDB = $ResNo1."-".$ResNo2;
	            ...
	            //[1-1-2]불일치 - ID사용불가
		            ...
		            $str_UserID = "<font color='red'>중복된 아이디</font>";
		            $error_count++;
		        }
		    //[2]없는경우 - ID사용가능
		    }else{
