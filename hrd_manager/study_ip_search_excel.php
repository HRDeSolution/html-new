...
$Colume = "a.Seq, a.ServiceType, a.ID, a.LectureStart, a.LectureEnd, a.Progress, 
				a.PassOK, a.certCount, a.StudyEnd, a.StudyIP,
				b.ContentsName,
				c.Name, c.Depart, 
...
$filename = "IP모니터링_".date('Ymd');

$TOT_NO2 = $TOT_NO + 1;

//cell border
$objPHPExcel->getActiveSheet()->getStyle('A1:M'.$TOT_NO2)->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
//align
$objPHPExcel->getActiveSheet()->getStyle('A1:M'.$TOT_NO2)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
$objPHPExcel->getActiveSheet()->getStyle('A1:M'.$TOT_NO2)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
$objPHPExcel->getActiveSheet()->getStyle('A1:M'.$TOT_NO2)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);


//1행 처리
$objPHPExcel->getActiveSheet()->getStyle('A1:M1')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('E8E8E8E8');
$objPHPExcel->getActiveSheet()->getCell('A1')->setValue("번호");
// $objPHPExcel->getActiveSheet()->getCell('B1')->setValue("구분");
$objPHPExcel->getActiveSheet()->getCell('B1')->setValue("영업자");
$objPHPExcel->getActiveSheet()->getCell('C1')->setValue("영업자 소속");
$objPHPExcel->getActiveSheet()->getCell('D1')->setValue("사업자");
$objPHPExcel->getActiveSheet()->getCell('E1')->setValue("사업자 주소");
$objPHPExcel->getActiveSheet()->getCell('F1')->setValue("ID");
$objPHPExcel->getActiveSheet()->getCell('G1')->setValue("이름");
$objPHPExcel->getActiveSheet()->getCell('H1')->setValue("과정명");
$objPHPExcel->getActiveSheet()->getCell('I1')->setValue("수강기간");
$objPHPExcel->getActiveSheet()->getCell('J1')->setValue("첨삭완료");
$objPHPExcel->getActiveSheet()->getCell('K1')->setValue("수강IP");
$objPHPExcel->getActiveSheet()->getCell('L1')->setValue("중복IP");
$objPHPExcel->getActiveSheet()->getCell('M1')->setValue("연속IP");
...
{
	$ip_term_range = 5; // ip패턴 범위(기준 아이피로 부터 +-5까지 범위로 패턴을 검색)
while($ROW = mysqli_fetch_array($QUERY))
	...
	// 변수 초기화
	$study_pattern_cnt = 0; // 수강ip 유사패턴 개수
	/**
	 * 유사 IP 패턴 검사 (헤비쿼리를 유발하지 않게 하기 위해 하단 4개의 조건문으로 분기)
	 * 타겟이 되는 ip의 뒷자리 기준으로 +-지정 범위만큼 앞뒤 범위로 구함. (예. ip_term_range = 5, 기준ip : 123.123.123.51 => 검색범위 : 123.123.123.46 ~ 123.123.123.56)
	*/ 
	...
	$objPHPExcel->getActiveSheet()->getCell('B'.$i)->setValueExplicit($SalesName, PHPExcel_Cell_DataType::TYPE_STRING);
	$objPHPExcel->getActiveSheet()->getCell('C'.$i)->setValueExplicit($SalesTeam, PHPExcel_Cell_DataType::TYPE_STRING);
	$objPHPExcel->getActiveSheet()->getCell('D'.$i)->setValueExplicit($CompanyName, PHPExcel_Cell_DataType::TYPE_STRING);
	$objPHPExcel->getActiveSheet()->getCell('E'.$i)->setValueExplicit($Address01." " . $Address02, PHPExcel_Cell_DataType::TYPE_STRING);
	...
	$objPHPExcel->getActiveSheet()->getCell('H'.$i)->setValueExplicit($ContentsName, PHPExcel_Cell_DataType::TYPE_STRING);
	$objPHPExcel->getActiveSheet()->getCell('I'.$i)->setValueExplicit($LectureDate, PHPExcel_Cell_DataType::TYPE_STRING);
	$objPHPExcel->getActiveSheet()->getCell('J'.$i)->setValueExplicit($Tutor_limit_day2, PHPExcel_Cell_DataType::TYPE_STRING);
	$objPHPExcel->getActiveSheet()->getCell('K'.$i)->setValueExplicit($StudyIP, PHPExcel_Cell_DataType::TYPE_STRING);
		
	if ($study_duple_cnt > 0) $duple_tag = '중복'; 
	if ($study_pattern_cnt > 0) $pattern_tag = '유사패턴';
	
	$objPHPExcel->getActiveSheet()->getCell("L" . $i)->setValueExplicit($duple_tag, PHPExcel_Cell_DataType::TYPE_STRING);
	$objPHPExcel->getActiveSheet()->getCell("M" . $i)->setValueExplicit($pattern_tag, PHPExcel_Cell_DataType::TYPE_STRING);
