...
$CsrfToken = Replace_Check($CsrfToken);
...
if(!$Mailling)      $Mailling = "N";
if(!$ACS)           $ACS = "N";
if(!$EduManager)    $EduManager = "N";
if(!$ResNo)         $ResNo = "";
if(!$ProtectID)     $ProtectID = "N";
...
<?
	...
	//주민번호 중복체크
	if($ResNo) {
		$Sql = "SELECT COUNT(idx) FROM Member WHERE ResNo=HEX(AES_ENCRYPT('$InputResNo','$DB_Enc_Key')) AND MemberOut = 'N'";
		$Result = mysqli_query($connect, $Sql);
		$Row = mysqli_fetch_array($Result);
		$ResNo_count = $Row[0];
...
if($mode=="edit") {
	//주민번호 중복체크
	if($ResNo) {
		$Sql = "SELECT COUNT(idx) FROM Member WHERE ResNo=HEX(AES_ENCRYPT('$InputResNo','$DB_Enc_Key')) AND MemberOut = 'N' AND idx != $idx";
		$Result = mysqli_query($connect, $Sql);
		$Row = mysqli_fetch_array($Result);
		$ResNo_count = $Row[0];
...
<?
	...
	$Sql = "UPDATE Member SET  MemberType='$MemberType', Name='$Name', Gender='$Gender', BirthDay=$BirthDay_enc, Email=$Email_enc, Tel=$Tel_enc, Mobile=$Mobile_enc, Zipcode='$Zipcode', 
			                   Address01='$Address01', Address02='$Address02', NameEng='$NameEng', CompanyCode='$CompanyCode', Depart='$Depart', Position='$Position', Etc01='$Etc01', Etc02='$Etc02',
                               Mailling='$Mailling', ACS='$ACS', Marketing='$Marketing', EduManager='$EduManager', ResNo=$ResNo_enc, ProtectID='$ProtectID', CardName='$CardName', CardNumber='$CardNumber', UseYN='$UseYN' 
			WHERE idx=$idx";
	$Row = mysqli_query($connect, $Sql);
