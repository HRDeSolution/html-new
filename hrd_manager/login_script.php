...
$Passwd2 = Replace_Check_XSS2($Passwd2); //2차인증

if(strpos($_SERVER["HTTP_HOST"],"tutor") > -1) $Dept_str = " AND a.Dept='C'"; //첨삭강사로 로그인시
else $Dept_str = " AND (a.Dept='A' OR a.Dept='B')"; //일반관리자 또는 영업사원으로 로그인시

$Sql = "SELECT a.ID, a.Name, a.Team, a.Mobile, b.Dept, b.DeptName, b.DeptString, b.TopMenuGrant, b.SubMenuGrant
        FROM StaffInfo AS a
...
if($Row) {
	...
	if ($ID != 'admin05') { // 예외처리
		if (empty($Passwd2)) {
			echo "<script>alert('2차인증 정보를 입력하세요.');</script>";
			exit;
		} else {
			$phone_rear_num = substr($Row['Mobile'], -4);

			if ($phone_rear_num != $Passwd2) {
				echo "<script>alert('2차인증 정보가 일치하지 않습니다.');</script>";
				exit;
			}
		}
	}
	...
	if($IDSave=="Y") setCookie("AdminSavedID",$ID,time()+15768000,"/");
	else setCookie("AdminSavedID","",0,"/");
