...
if($EduManager!="Y" && $EduManager!="N") $EduManager="N";
if(!$Pwd) $Pwd = "1111";

$enc_pwd = encrypt_SHA256($Pwd); //비밀번호 암호화

if(!$OpenChapter) $OpenChapter = "1";

//20250630.YEON.비환급일경우에는 실시회차가 무조건 "0"
if($ServiceType == "W") $OpenChapter = 0;
...
//현재 회원중에 주민번호가 일치하는 회원이 없는 경우 신규 회원으로 처리
if(!$UserID) {
	...
	if($ID_Count>0) {
	    //회사 아이디+생년월일에서 뒤에 1부터 50까지 붙여 동일 아이디가 있는지 확인
		for($i=1;$i<=50;$i++) { 
			$UserID2 = $UserID.$i;
...
if($Member_count>0)  {
	...
	$Sql = "INSERT INTO Member(ID, Pwd, Name, BirthDay, Gender, Email, Mobile, CompanyCode, Depart, Mailling, ACS, MemberOut, Sleep, UseYN, RegDate, EduManager, ResNo, Zipcode, trneeSe, IrglbrSe) 
			VALUES('$UserID', '$enc_pwd', '$Name', $BirthDay_enc, '$Gender', $Email_enc, $Mobile_enc, '$CompanyCode', '$Depart', 'Y', 'Y', 'N', 'N', 'Y', NOW(), '$EduManager', $ResNo_enc, '-', '$trneeSe', '$IrglbrSe')";
}
...
if($StudyKey_check>0) {
	...
	$Sql_Input = "INSERT INTO Study(Seq, LectureTerme_idx, LectureCode, ServiceType, Tutor, ID, CompanyCode, LectureStart, LectureEnd, LectureReStudy, 
				  ...
				  VALUES($max_Seq, $LectureTerme_idx, '$LectureCode', '$ServiceType', '$Tutor', '$UserID', '$CompanyCode', '$LectureStart', '$LectureEnd', '$LectureReStudy', 
				         $Progress, $OpenChapter, '$StudyKey', 1, 0, '$LoginAdminID', NOW(), '$SalesID')";
...
if($error_count>0) {
	...
	$msg = "<font color='red'>오류</font>".$msg01.$msg02.$msg04;
}else{
