...
$objPHPExcel->getActiveSheet()->getCell('G3')->setValue('총훈련비');
$objPHPExcel->getActiveSheet()->getCell('H3')->setValue('자부담금'); // 열 추가

$objPHPExcel->getActiveSheet()->getStyle('A3:G3')->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
$objPHPExcel->getActiveSheet()->getStyle('A3:H3')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('E8E8E8E8');
...
$objPHPExcel->getActiveSheet()->getStyle('A4:H'.$CellCount)->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
$objPHPExcel->getActiveSheet()->getStyle('A4:H'.$CellCount)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
...
// 		$objPHPExcel->getActiveSheet()->getCell('B'.$k)->setValueExplicit($ContentsName, PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->getCell('B'.$k)->setValueExplicit('디지털원격훈련 아카이브', PHPExcel_Cell_DataType::TYPE_STRING);
		...
		$objPHPExcel->getActiveSheet()->getCell('G'.$k)->setValueExplicit('140,000', PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->getCell('H'.$k)->setValueExplicit('14,000', PHPExcel_Cell_DataType::TYPE_STRING); // 자부담금 추가
...
$PriceT = 140000 * ($i-1);
$SelfPriceT = 14000 * ($i-1);

$objPHPExcel->getActiveSheet()->getCell('A'.$k2)->setValue('총 훈련비 합계');
$objPHPExcel->getActiveSheet()->getCell('G'.$k2)->setValueExplicit(number_format($PriceT,0), PHPExcel_Cell_DataType::TYPE_STRING);
$objPHPExcel->getActiveSheet()->getCell('H'.$k2)->setValueExplicit(number_format($SelfPriceT,0), PHPExcel_Cell_DataType::TYPE_STRING);
