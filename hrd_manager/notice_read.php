...
<?
$Sql = "SELECT * FROM Notice WHERE idx=$idx AND Del='N'";
...
if($Row) {
	...
	if($UseYN=="Y") $UseYN_MSG = "<font color='blue'>사용</font>";
	else $UseYN_MSG = "<font color='red'>미사용</font>";
}
?>
...
function DelOk() {
	...
	if(del_confirm==true) {
        ...
        <div class="contentBody">
        	<h2>공지사항</h2>
            <div class="conZone">
    		<form name="DeleteForm" method="post" action="notice_script.php" enctype="multipart/form-data" target="ScriptFrame">
            ...
            <table width="100%" cellpadding="0" cellspacing="0" class="view_ty01">
    			...
    			<tr>
    				<th>첨부 파일 1</th>
    				<td> 
    				<?if($FileName1) { ?><A HREF="./download.php?idx=<?=$idx?>&code=Notice&file=1" target="ScriptFrame"><?=$RealFileName1?></A><?}?>
    				</td>
    			</tr>
    			<tr>
    				<th>첨부 파일 2</th>
    				<td> 
    				<?if($FileName2) { ?><A HREF="./download.php?idx=<?=$idx?>&code=Notice&file=2" target="ScriptFrame"><?=$RealFileName2?></A><?}?>
    				</td>
    			</tr>
    			<tr>
    				<th>첨부 파일 3</th>
    				<td> 
    				<?if($FileName3) { ?><A HREF="./download.php?idx=<?=$idx?>&code=Notice&file=3" target="ScriptFrame"><?=$RealFileName3?></A><?}?>
    				</td>
    			</tr>
    			<tr>
    				<th>첨부 파일 4</th>
    				<td> 
    				<?if($FileName4) { ?><A HREF="./download.php?idx=<?=$idx?>&code=Notice&file=4" target="ScriptFrame"><?=$RealFileName4?></A><?}?>
    				</td>
    			</tr>
    			<tr>
    				<th>첨부 파일 5</th>
    				<td> 
    				<?if($FileName5) { ?><A HREF="./download.php?idx=<?=$idx?>&code=Notice&file=5" target="ScriptFrame"><?=$RealFileName5?></A><?}?>
    				</td>
    			</tr>
    			<tr>
    				<th>등록일</th>
    			...
    			<tr>
    				...
    				<td height="28">
        				<table border="0" width="970px">
        					<tr><td style="border:0px"><?=$Content?></td></tr>
        				</table>
    				</td>
    			</tr>
            </table>
