...
$Colume = "a.Seq, a.ServiceType, a.ID, a.LectureStart, a.LectureEnd, a.Progress, a.PassOK, a.certCount, a.StudyEnd, a.LectureCode, a.CompanyCode
          ,b.ContentsName
    	  ,c.Name, c.Depart, AES_DECRYPT(UNHEX(c.Email),'$DB_Enc_Key') AS Email, AES_DECRYPT(UNHEX(c.Mobile),'$DB_Enc_Key') AS Mobile ,c.ACS
...
if($CyberEnabled=="Y" && $CyberURL) $DomainURL = $CyberURL;
else $DomainURL = $SiteURL;
...
$TemplateMessage = addslashes($TemplateMessage);
...
if($send_mode=="sms" && $ACS == "Y" && $SendSMS == "Y") {
	...
	$Sql = "INSERT INTO SmsSendLog(idx, ID, Study_Seq, Massage, Code, Mobile, InputID, RegDate)
            VALUES($maxno, '$ID', $Seq, '$TemplateMessage', '', '$phone', '$LoginAdminID', NOW())";
	$Row = mysqli_query($connect, $Sql);
	
	if($Massage == $TemplateMessage)   $send = aligo_mms_send ($phone, $MessageMode, $MessageMode, $TemplateMessage, $callback, $TemplateCode);
	else   $send = aligo_send($phone, $Massage, $callback);
...
if($send_mode=="email") {
	...
	$Massage = "<div style='width:800px; margin:0 auto; padding-bottom:40px;'>
                    ...
                    <p style='font-size:21px; font-weight:bold;'>안녕하세요!<br />".$SiteName." 교육지원팀입니다.</p>
...
if($Row && $send) $result_msg = "<font color='blue'>성공</font>";
else $result_msg = "<font color='red'>실패</font>";
