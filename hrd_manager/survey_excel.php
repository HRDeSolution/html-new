...
$objPHPExcel->getActiveSheet()->getCell('B1')->setValue("과정명");
$objPHPExcel->getActiveSheet()->getCell('C1')->setValue("별점");
$objPHPExcel->getActiveSheet()->getCell('D1')->setValue("아이디");
$objPHPExcel->getActiveSheet()->getCell('E1')->setValue("제목");
$objPHPExcel->getActiveSheet()->getCell('F1')->setValue("등록IP");
$objPHPExcel->getActiveSheet()->getCell('G1')->setValue("등록일");
$objPHPExcel->getActiveSheet()->getCell('H1')->setValue("조회수");
...
if($QUERY && mysqli_num_rows($QUERY)){
    while($ROW = mysqli_fetch_array($QUERY)){
        extract($ROW);
        $Star = StarPointViewE($StarPoint, 'excel');
        
        $objPHPExcel->getActiveSheet()->getCell('A'.$i)->setValueExplicit($k, PHPExcel_Cell_DataType::TYPE_NUMERIC);
        $objPHPExcel->getActiveSheet()->getCell('B'.$i)->setValueExplicit($ContentsName, PHPExcel_Cell_DataType::TYPE_STRING);
        $objPHPExcel->getActiveSheet()->getCell('C'.$i)->setValueExplicit($Star, PHPExcel_Cell_DataType::TYPE_STRING);
        $objPHPExcel->getActiveSheet()->getCell('D'.$i)->setValueExplicit($ID, PHPExcel_Cell_DataType::TYPE_STRING);
        $objPHPExcel->getActiveSheet()->getCell('E'.$i)->setValueExplicit($Title, PHPExcel_Cell_DataType::TYPE_STRING);
        $objPHPExcel->getActiveSheet()->getCell('F'.$i)->setValueExplicit($IP, PHPExcel_Cell_DataType::TYPE_STRING);
        $objPHPExcel->getActiveSheet()->getCell('G'.$i)->setValueExplicit($RegDate, PHPExcel_Cell_DataType::TYPE_STRING);
        $objPHPExcel->getActiveSheet()->getCell('H'.$i)->setValueExplicit($ViewCount, PHPExcel_Cell_DataType::TYPE_STRING);
