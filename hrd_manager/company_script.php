...
<?
	...
	$Del = "N";
	$Sql = "INSERT INTO Company (idx, CompanyScale, CompanyCode, CompanyName, CompanyID, HRD, Ceo, Uptae, Upjong, Zipcode, Address01, Address02, Tel, Tel2, Fax, Fax2, 
				                Email, BankName, BankNumber, CSEnabled, CyberEnabled, HomePage, CyberURL, EduManager, EduManagerPhone, EduManagerEmail, SalesManager, RegDate, Remark, Del, SendSMS) 
			VALUES ($maxno, '$CompanyScale', '$CompanyCode', '$CompanyName', '$CompanyID', '$HRD', '$Ceo', '$Uptae', '$Upjong', '$Zipcode', '$Address01', '$Address02', '$Tel', '$Tel2', '$Fax', '$Fax2', 
				    '$Email', '$BankName', '$BankNumber', '$CSEnabled', '$CyberEnabled', '$HomePage', '$CyberURL', '$EduManager', '$EduManagerPhone', '$EduManagerEmail', '$SalesManager', NOW(), '$Remark','$Del', '$SendSMS')";
...
//삭제
if($mode=="del") {
    $Sql = "INSERT INTO CompanyDeleteLog
                (idx , CompanyScale ,CompanyCode ,CompanyName ,CompanyID ,HRD ,Ceo ,Uptae ,Upjong ,Zipcode ,Address01 ,Address02 ,Tel ,Tel2 ,Fax ,Fax2 ,
                Email ,BankName ,BankNumber ,CSEnabled ,CyberEnabled ,HomePage ,CyberURL ,EduManager ,EduManagerPhone ,EduManagerEmail ,SalesManager ,RegDate ,Remark ,Del ,SendSMS ,DelDate ,DelAdminID)
            SELECT	idx , CompanyScale ,CompanyCode ,CompanyName ,CompanyID ,HRD ,Ceo ,Uptae ,Upjong ,Zipcode ,Address01 ,Address02 ,Tel ,Tel2 ,Fax ,Fax2 ,
            		Email ,BankName ,BankNumber ,CSEnabled ,CyberEnabled ,HomePage ,CyberURL ,EduManager ,EduManagerPhone ,EduManagerEmail ,SalesManager ,RegDate ,Remark ,Del ,SendSMS , NOW(), '$LoginAdminID'
            FROM Company
            WHERE idx = $idx;";
    $Row = mysqli_query($connect, $Sql);
	
	$Sql2 = "DELETE FROM Company WHERE idx=$idx";
