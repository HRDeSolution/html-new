...
if($sw){
	if($col=="") {
		...
		if($col=="a.Mobile"){
			$Mobile_enc = "HEX(AES_ENCRYPT('$sw','$DB_Enc_Key'))";
			$where[] = "$col = $Mobile_enc";
		}else{
		    $where[] = "$col LIKE '%$sw%'";
		}
...
if($LoginAdminDept == "B") {
	...
	if($DeptString) {
		...
		//	echo $SQL."<br>";
			...
			// echo "'Dept_String : '".$Dept_String."<br>";
...
<div class="contentBody">
    ...
    <div class="conZone">
		<form name="search" method="get">
            <div class="searchPan">
            	<select name="col">
					...
					<option value="a.LastLogin" <?if($col=="a.LastLogin") { echo "selected";}?>>마지막로그인 1년이상</option>
				</select>
        ...
        <table width="100%" cellpadding="0" cellspacing="0" class="list_ty01 gapT20">
			...
			<?
			$SQL = "SELECT  a.*, b.CompanyName, AES_DECRYPT(UNHEX(a.Email),'$DB_Enc_Key') AS Email, 
                    ...
                    LEFT OUTER JOIN StaffInfo AS c ON b.SalesManager=c.ID 
                    $where $str_orderby LIMIT $PAGE_CLASS->page_start, $page_size";
            //if($LoginAdminID  == "admin05") echo $SQL;
			$QUERY = mysqli_query($connect, $SQL);
			if($QUERY && mysqli_num_rows($QUERY)){
				while($ROW = mysqli_fetch_array($QUERY)){
					extract($ROW);
