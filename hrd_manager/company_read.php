...
<SCRIPT LANGUAGE="JavaScript">
    ...
    $(document).ready(function(){
    	$("#SearchYear").change(function(){
    		$("#LoginHistoryA").load(location.href+" #LoginHistoryA",{SearchYear: $('#SearchYear').val()});
    	});
	});
</SCRIPT>
	<div class="contentBody">
        ...
        <div class="conZone">
			...
			<?if($AdminWrite=="Y") {?>
               	<button type="button" class="btn btn_Blue" onclick="location.href='<?=$PageName?>_write.php?mode=edit&idx=<?=$idx?>&col=<?=$col?>&sw=<?=urlencode($sw)?>'">정보 수정</button>
				<button type="button" class="btn btn_DGray line" onclick="DelOk()">삭제</button>
			<?}?>
			...
			<div class="btnAreaTl02">
				...
				?>
				<select name="SearchYear" id="SearchYear" style="padding: 1px 5px;padding-right: 25px;position: relative;background: #fff url(./images/bg_select.svg) no-repeat right center;background-size: auto 30px;">
			...
			<div id="LoginHistoryA" style="left:10px; top:20px; width:100%; height:400px; z-index:1; overflow: auto; overflow-x:hidden;">
    			<table width="100%" cellpadding="0" cellspacing="0" class="list_ty01 gapT20">
                    ...
                    if($SearchYear == 'all'){
                        $where = "WHERE a.CompanyCode='$CompanyCode' AND a.ServiceType='A' ";
                    }else{
                        $where = "WHERE a.CompanyCode='$CompanyCode' AND a.ServiceType='A' AND YEAR(a.LectureStart)= '$SearchYear' ";
                    }
                    ...
                    $SQL = "SELECT DISTINCT a.LectureStart, a.LectureEnd
                            ...
                            $where
                            ORDER BY a.LectureStart DESC, c.ContentsName DESC ";
                    //echo $SQL;
                    ...
                    if($QUERY && mysqli_num_rows($QUERY)){
    				    while($ROW = mysqli_fetch_array($QUERY)){
            				...
            				$SQL2 = "SELECT DISTINCT(LectureCode), 
            				        ...
            				        FROM Study AS a 
                                    WHERE a.LectureStart='$LectureStart' AND a.LectureEnd='$LectureEnd' AND a.CompanyCode='$CompanyCode' ";
        	...
        	<div class="btnAreaTl02">
        		<span class="fs16b fc333B sub_title2">수강생 정보</span>
        		<!-- Brad (2021.11.28) : 엑셀출력 기능 추가 -->
        		<button type="button" name="ExcelOutBtn" id="ExcelOutBtn" class="btn btn_Green line" onClick="location.href='<?=$ReadPage?>_excel.php?CompanyCode=<?= $CompanyCode ?>'"><i class="fas fa-file-excel"></i> 검색항목 엑셀 출력</button>
        	</div>
        	...
        	<div class="btnAreaTl02"><span class="fs16b fc333B sub_title2">수강 내역</span></div>
			<div id="StudyHistory" style="left:10px; top:20px; width:100%; height:500px; z-index:1; overflow: auto; overflow-x:hidden;">
    			<table width="100%" cellpadding="0" cellspacing="0" class="list_ty01 gapT20">
            		...
            		$Sql = "SELECT COUNT(a.Seq) FROM $JoinQuery WHERE a.CompanyCode='$CompanyCode'";
            		//echo $where;
            		$Result = mysqli_query($connect, $Sql);
            		$Row = mysqli_fetch_array($Result);
            		$TOT_NO = $Row[0];
            		...
            		$i = 1;
            		$SQL = "SELECT $Colume FROM $JoinQuery WHERE a.CompanyCode='$CompanyCode' ORDER BY a.LectureStart DESC";
					// echo $SQL;
            		$QUERY = mysqli_query($connect, $SQL);
            		if($QUERY && mysqli_num_rows($QUERY)){
            			while($ROW = mysqli_fetch_array($QUERY)){
            		...
            		?>
            		<tr>
                    	<td><?=$i?><br /><?=$ServiceType2_array[$ServiceType]?></td>
