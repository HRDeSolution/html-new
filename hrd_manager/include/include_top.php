...
<link rel="stylesheet" href="./css/style.css?v250512" type="text/css">
...
<SCRIPT LANGUAGE="JavaScript">
    ...
    function changeSelect2Style(){
		$(".select2-container").attr("style","width:400px;");
    	$(".select2-container .select2-selection--single").attr("style","width:400px; height:36px !important; padding:4px 0px; border:1px solid #c6c6c6; border-radius:3px;");
    	$(".select2-selection__arrow").attr("style","margin-top:3px;");
    	$(".select2-label").attr("style","margin-right:5px;");
    }
</SCRIPT>
</head>
...
<div id="wrap">
	<!-- Header -->
    <div class="Header">
    	<div class="logo"><a href="../hrd_manager/main.php"><img src="../hrd_manager/images/logo_hrde.png" alt="logo"></a></div>
		<div class="inner">
			<!-- top -->
			<?
			switch (date("w")) {
			...
			<div class="mainMenu">
				<ul class="area">
					<?
					if(in_array('A',$LoginAdminTopMenuGrant_array) && $LoginAdminDept != "C"){
					?>
					...
					if($LoginAdminDept=="C") {
					?>
					<li><a href="live_class.php" <?if($MenuType=="L") {?>class="show"<?}?>>실시간 강의</a></li>
					<?}?>
					...
					<li><a href="<?=$Top_Link_E_page?>" <?if($MenuType=="E") {?>class="show"<?}?>>아카이브</a></li>
					<?}?>
					<?if(in_array('F',$LoginAdminTopMenuGrant_array)){?>
					<li><a href="<?=$Top_Link_F_page?>" <?if($MenuType=="F") {?>class="show"<?}?>>커뮤니티관리</a></li>
					<?}?>
