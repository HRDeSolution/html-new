...
if($mode!="new") {
	$Sql = "SELECT  *, AES_DECRYPT(UNHEX(ResNo),'$DB_Enc_Key') AS ResNo, AES_DECRYPT(UNHEX(Email),'$DB_Enc_Key') AS Email, AES_DECRYPT(UNHEX(Mobile),'$DB_Enc_Key') AS Mobile, 
                    AES_DECRYPT(UNHEX(Tel),'$DB_Enc_Key') AS Tel, AES_DECRYPT(UNHEX(BirthDay),'$DB_Enc_Key') AS BirthDay,
                    (SELECT CompanyName FROM Company WHERE CompanyCode=Member.CompanyCode LIMIT 0,1) AS CompanyName
            FROM Member
            WHERE idx=$idx AND MemberOut='N'";
	...
	if($Row) {
    	...
    	$CardName = $Row['CardName']; //내일배움카드명
    	$CardNumber = $Row['CardNumber']; //내일배움카드 번호
    	$RecomID = $Row['RecomID']; //추천인아이디
...
if(!$ProtectID) $ProtectID = "Y";
if(!$Marketing) $Marketing = "N";
?>
                ...
                <table width="100%" cellpadding="0" cellspacing="0" class="view_ty01">
					...
					<tr>
                  	...
                  	<tr>
                        <th>아이디</th>
                        <td>
                        	<input name="ID" id="ID" type="text"  size="20" value="<?=$ID?>">&nbsp;&nbsp;
					...
					<tr>
				  	...
				  	<tr>
                        <th>주소</th>
                        <td>
                        	<input name="Zipcode" id="Zipcode" type="text"  size="10" value="<?=$Zipcode?>">&nbsp;&nbsp;
                        	<button type="button" onclick="ZipcodeFind();" class="btn round btn_LBlue line"><i class="xi-maker"></i> 주소 찾기</button>
    						<p>
    							<input name="Address01" id="Address01" type="text"  size="60" value="<?=$Address01?>" placeholder="주소"> 
    							<input name="Address02" id="Address02" type="text"  size="60" value="<?=$Address02?>" placeholder="나머지 주소">
    						</p>
    					</td>
                  	</tr>
				  	<tr>
				  	...
				  	<tr>
                        <th>회사명</th>
                        <td>
                        	<input name="CompanyName" id="CompanyName" type="text"  size="40" value="<?=$CompanyName?>">&nbsp;
    						<button type="button" onclick="MemberCompanySearch();" class="btn round btn_LBlue line"><i class="xi-search"></i> 검색</button>&nbsp;&nbsp;
    						<span id="company_search_result"></span>
    					</td>
                  	</tr>
				  	<tr>
                        <th>부서</th>
				  	...
				  	<tr>
                        <th>관심분야</th>
                        <td>
        					<input type="checkbox" name="Etc01Temp" id="Etc01_01" value="일반과정" <?if($mode!="new"){ if(in_array("일반과정",$Etc01_Arrary)) {?>checked<?} }?> style="width:18px" /> <label for="Etc01_01">일반과정</label>&nbsp;&nbsp;&nbsp;&nbsp;
        					<input type="checkbox" name="Etc01Temp" id="Etc01_02" value="유아교육" <?if($mode!="new"){ if(in_array("유아교육",$Etc01_Arrary)) {?>checked<?} }?> style="width:18px" /> <label for="Etc01_02">유아교육</label>&nbsp;
        					<input type="checkbox" name="Etc01Temp" id="Etc01_03" value="자격증" <?if($mode!="new"){ if(in_array("자격증",$Etc01_Arrary)) {?>checked<?} }?> style="width:18px" /> <label for="Etc01_03">자격증</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        					<input type="checkbox" name="Etc01Temp" id="Etc01_04" value="개인환급" <?if($mode!="new"){ if(in_array("개인환급",$Etc01_Arrary)) {?>checked<?} }?> style="width:18px" /> <label for="Etc01_04">개인환급</label>&nbsp;
        					<input type="checkbox" name="Etc01Temp" id="Etc01_05" value="기업환급" <?if($mode!="new"){ if(in_array("기업환급",$Etc01_Arrary)) {?>checked<?} }?> style="width:18px" /> <label for="Etc01_05">기업환급</label>
    					</td>
                  	</tr>
				  	<tr>
                        <th>가입경로</th>
                        <td>
        					<input type="checkbox" name="Etc02Temp" id="Etc02_01" value="인터넷검색" <?if($mode!="new"){ if(in_array("인터넷검색",$Etc02_Arrary)) {?>checked<?} }?> style="width:18px" /> <label for="Etc02_01">인터넷검색</label>&nbsp;
        					<input type="checkbox" name="Etc02Temp" id="Etc02_02" value="SNS" <?if($mode!="new"){ if(in_array("SNS",$Etc02_Arrary)) {?>checked<?} }?> style="width:18px" /> <label for="Etc02_02">SNS</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        					<input type="checkbox" name="Etc02Temp" id="Etc02_03" value="직장동료소개" <?if($mode!="new"){ if(in_array("직장동료소개",$Etc02_Arrary)) {?>checked<?} }?> style="width:18px" /> <label for="Etc02_03">직장동료소개</label>&nbsp;
        					<input type="checkbox" name="Etc02Temp" id="Etc02_04" value="홍보책자" <?if($mode!="new"){ if(in_array("홍보책자",$Etc02_Arrary)) {?>checked<?} }?> style="width:18px" /> <label for="Etc02_04">홍보책자</label>&nbsp;
        					<input type="checkbox" name="Etc02Temp" id="Etc02_05" value="기타" <?if($mode!="new"){ if(in_array("기타",$Etc02_Arrary)) {?>checked<?} }?> style="width:18px" /> <label for="Etc02_05">기타</label>
						</td>
                  	</tr>
				  	<tr>
                        <th>메일링 서비스</th>
                  	...
                  	<tr>
                        <th>추천 아이디</th>
                        <td><?if($RecomID) echo $RecomID; else echo "-";?></td>
                  	</tr>
                </table>
			</form>
