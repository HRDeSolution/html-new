...
##-- 검색 등록수
$Sql = "SELECT COUNT(*) FROM LectureEvent a LEFT JOIN `Member` b ON a.ID = b.ID $where";
$Result = mysqli_query($connect, $Sql);
$Row = mysqli_fetch_array($Result);
$TOT_NO = $Row[0];


$filename = "이벤트리스트_".date('Ymd');
...
$objPHPExcel->getActiveSheet()->getCell('E1')->setValue("스테이지");
$objPHPExcel->getActiveSheet()->getCell('F1')->setValue("카운트");
...
if($QUERY && mysqli_num_rows($QUERY)) {
	while($ROW = mysqli_fetch_array($QUERY)) {
		...
		$objPHPExcel->getActiveSheet()->getCell('E'.$i)->setValueExplicit($Stage."Days", PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->getCell('F'.$i)->setValueExplicit($StageCount, PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->getCell('G'.$i)->setValueExplicit($CounselStatus_array[$Status], PHPExcel_Cell_DataType::TYPE_STRING);
