...
for($i=0;$i<$CreatCount;$i++) {
	...
	//Brad (2021.12.01) : test ID 형식변경
	//$Sql = "SELECT MAX(RIGHT(ID,5)) FROM Member WHERE ID LIKE 'ekedu_____'";
	$Sql = "SELECT MAX(RIGHT(ID,5)) FROM Member WHERE ID LIKE 'simsa_____'";
	...
	if($MAX_ID) {
		...
		$UserID = "simsa".str_pad($ID_Num , 5, '0', STR_PAD_LEFT);
	...
	$Zipcode = "08501";
	$Address01 = "경기도 용인시 기흥구 흥덕1로 13";
	$Address02 = "흥덕IT밸리 타워동 104호, 105호";
	$NameEng = "";
	$CompanyCode = "5918702242";
	$Depart = "";
	$Position = "";
	$Etc01 = "";
	$Etc02 = "";
	...
	// Brad (2021.12.01) : Pass Change Field Default 'Y'
	$Sql_m = "INSERT INTO Member(ID, Pwd, Name, BirthDay, Gender, Email, Tel, Mobile, Zipcode, Address01, Address02, 
			                     NameEng, CompanyCode, Depart, Position, Etc01, Etc02, Mailling, ACS, MemberOut, Sleep, UseYN, RegDate, EduManager, ResNo, TestLectureCode, TestID, ProtectID, PassChange) 
			 VALUES('$UserID', '$enc_pwd', '$Name', $BirthDay_enc, '$Gender', $Email_enc, $Tel_enc, $Mobile_enc, '$Zipcode', '$Address01', '$Address02', 
			         '$NameEng', '$CompanyCode', '$Depart', '$Position', '$Etc01', '$Etc02', '$Mailling', '$ACS', '$MemberOut', '$Sleep', '$UseYN', NOW(), '$EduManager', $ResNo_enc, '$TestLectureCode', '$TestID', '$ProtectID', 'Y')";
	...
	if($QUERY_c && mysqli_num_rows($QUERY_c)){
		while($ROW_c = mysqli_fetch_array($QUERY_c)){
			...
			$Sql_c2 = "INSERT INTO Progress(ID, LectureCode, Study_Seq, Chapter_Seq, Contents_idx, ContentsDetail_Seq, LastStudy, Progress, StudyTime, UserIP, RegDate, TriggerYN, Chapter_Number)
                       VALUES('$UserID', '$LectureCode', $max_Seq, $Chapter_Seq, $Contents_idx, $ContentsDetail_Seq, '$ContentsURL', $ChapterProgress, $ProgressTime, '$UserIP', NOW(), '$TriggerYN', '$Chapter_Number')";
