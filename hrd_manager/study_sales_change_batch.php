...
<link rel="stylesheet" href="./css/style.css?v230807" type="text/css">
...
function ChangeBatch() {
	...
	var SalesID_text = $("#SalesID option:selected").text();
...
function BatchProcess(i) {
	...
	if(i<totalcount) {
		...
		if ($("input:checkbox[id='check_seq_"+i+"']").is(":checked") == false){
			...
			$("div[id='status']:eq("+i+")").load('./study_sales_change_batch_process.php',
		...
		<div class="Content">
	        <div class="contentBody">
        	    ...
        	    <div class="conZone">
    				...
    				<select name="SalesID" id="SalesID" style="width:200px; height:35px">
    					<option value="">영업담당자 선택</option>
    					<?
    					$SQL2 = "SELECT * FROM StaffInfo WHERE Dept='B' AND Del='N' ORDER BY Name ASC, ID ASC";
    					$QUERY2 = mysqli_query($connect, $SQL2);
    					if($QUERY2 && mysqli_num_rows($QUERY2)){
    						while($ROW2 = mysqli_fetch_array($QUERY2)){
						?>
    					<option value="<?=$ROW2['ID']?>"><?=$ROW2['Name']?> (<?=$ROW2['ID']?>)</option>
    					<?
    						}
    					}
    					?>
