...
$EVAL_CD         = Replace_Check_XSS2($EVAL_CD);
$EVAL_TYPE       = Replace_Check_XSS2($EVAL_TYPE);
$CLASS_TME       = Replace_Check_XSS2($CLASS_TME);


//인증저장내역이 있는지 확인
$Sql = "SELECT COUNT(*) FROM UserCertOTP WHERE ID='$LoginMemberID' AND LectureCode='$StudyLectureCode'";
...
if($CertCount<1) {
	...
	$Sql = "INSERT INTO UserCertOTP(Seq, ID, LectureCode, Study_Seq, CertDate, CertType, AGTID, COURSE_AGENT_PK, CLASS_AGENT_PK, m_Ret, m_retCD, m_trnID, m_trnDT, RegDate, UserIP, EvalCD, EvalType, ClassTime) 
            VALUES($maxno, '$LoginMemberID', '$LectureCode', $Study_Seq, '$CertDate', '$CertType', '$AGTID', '$COURSE_AGENT_PK', '$CLASS_AGENT_PK', '$m_Ret', '$m_retCD', '$m_trnID', '$m_trnDT', NOW(), '$UserIP', '$EVAL_CD', '$EVAL_TYPE', '$CLASS_TME')";
    ...
    $_SESSION['PlayStudyAuth_'.$Study_Seq.$Chapter_Seq] = "Y";
}
