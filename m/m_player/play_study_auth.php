...
$StudyLectureCode = Replace_Check_XSS2($StudyLectureCode);

$Sql    = "SELECT * FROM Study WHERE LectureCode = '$StudyLectureCode' AND Seq = $Study_Seq";
...
<script language='javascript'>
    ...
    function fnPopupmotp(){
        ...
        document.form_motp.action = "/m_player/motp.php?class_tme=<?=$Chapter_Number?>&EvalCd=<?=$EvalCd?>&LectureCode=<?=$LectureCode?>&COURSE_AGENT_PK=<?=$StudyLectureCode?>&CLASS_AGENT_PK=<?=$StudyLectureCode?>,<?=$LectureTerme_idx?>&Study_Seq=<?=$Study_Seq?>";
        document.form_motp.submit();	
    }

    function pSubOtpLog(m_retCD,m_trnID,m_trnDT) {
      CertCheckProc(m_trnDT);
    }
</script>
<?
//휴대폰 인증관련 ##################################################################################
...
$reqseq = $StudyLectureCode."_".date('YmdHis');
...
$returnurl = $SiteURL."/m_lib/CheckPlusSafe/checkplus_success_otp.php";	// 성공시 이동될 URL
$errorurl  = $SiteURL."/m_lib/CheckPlusSafe/checkplus_fail.php";		// 실패시 이동될 URL
...
$sReturnURL	 = $SiteURL."/m_lib/NiceIPIN/ipin_process_otp.php";		// 하단내용 참조
...
$sCPRequest = $StudyLectureCode."_".date('YmdHis');
...
해당 파라미터는 추가하실 수 없습니다. -->
	<input type="hidden" name="param_r1" value="<?=$StudyLectureCode?>">
...
#screenModal{overflow-x:hidden;overflow-y:auto;position:fixed;top:0;left:0;width:100%;height:100%;background:#f2f6f7;z-index:999;padding: 10px;}
#screenModal > div{width: 100%;margin:0 auto;border:1px solid #bfc3c4;background:#fff;}
#screenModal > div + div{margin-top:15px;}
#screenModal div.titleArea{overflow:hidden;width:100%;border-top:5px solid #0780c2;border-bottom:2px solid #2665ae;background:#fff; padding: 0 10px;}
#screenModal div.titleArea > div{overflow:hidden;width:100%;margin:20px auto;text-align:right;}
...
#screenModal div.titleArea > div > h1{ margin-bottom:10px;font-size: 24px;color:#343434;}
#screenModal div.titleArea > div > h2{font-size:20px;color:#565656;line-height:30px;}
#screenModal div.titleArea > div button{overflow:hidden;width:25px;height:25px;padding:0;border:none;background:none;}
#screenModal div.titleArea > div button img{width: 100%;}
#screenModal > div.caution{padding:30px 0;text-align:center;}
#screenModal > div.caution img{width: 90px;}
#screenModal > div.caution h1,#screenModal > div.caution strong{color:#3B74B6;}
#screenModal > div.caution h1{font-size:24px; margin:10px 0;}
#screenModal > div.caution p{margin:0 auto;line-height:22px;font-size:14px;padding:15px 10px;color:#000000;}
...
#screenModal > div.btnArea button img{width: 100%;}
...
div.mobileCert li{line-height:45px;padding:10px 20px;    display: flex ;}
...
div.mobileCert li button.btnPhone{background-image:url(https://winner797.kr/m_images/member/icon_phone.png);}
div.mobileCert li button.btnIpin{background-image:url(https://winner797.kr/m_images/member/icon_ipin.png);}
#screenModal div.titleArea > div > h2{margin:0;padding:0;text-align:left;}
</style>
...
<div id="screenModal" style="">
  <div class="titleArea">
    ...
    <button type="button" onclick="studyModalClose();">
        <img src="/m_images/study/btn_modalclose.png">
      </button>
      <h1>학습자 유의사항</h1>
  ...
  <div class="caution">
    <img src="/m_images/study/img_notice_big.png">
    <h1>주의사항</h1>
      ...
      <tbody>
        <tr>
          <td>
            ...
            <p>
              HRD 아카이브 본인인증 안내<br/>
                - 기업단체회원 최초 로그인 시 휴대폰 본인인증이 필요합니다.<br/>
                - 학습 진행시 1일 1회 mOTP 또는 휴대폰 본인인증이 필요합니다.<br/>
                <br/>
                일일진도제한 안내<br/>
                하루 최대 8시간을 초과할 수 없습니다. <br/>
                <br/>
                수료기준 안내<br/>
                HRD 아카이브 과정은 15시간(900분) 이상 수강 시 수료됩니다.<br/>
                - 최소 훈련시간인 15시간 충족 시 자동 수료됩니다. <br/>
                - 평가와 과제는 없습니다.<br/>
                -본인인증을 통해 과정에 진입하며 실제 과정을 수강한 시간만 훈련시간으로 인정됩니다. <br/>
                <br/>
                수료기준에 도달한 경우에만 고용노동부로부터 훈련비용의 지원을 받을 수 있습니다. <br/>
            </p>
          </td>
        </tr>
      </tbody>
    ...
    <table width="100%">
      <tbody>
        <tr>
          <td>
            <h1>[부정훈련 금지서약]</h1>
            <p>1. 수강생은/는 타인에게 회원정보(ID, 비밀번호 등)을 알려주지 않으며, 다른사람이 본인을 대신하여 교육을 수강하는 일이 없을 것을 서약합니다. <br>
            </p>
    ...
    <br>
    <input type="checkbox" id="agreeCaution" style="width: 20px; height: 20px;margin-top: -7px; margin-right: 7px;">
  ...
  <div class="mobileCert">
    ...
    <ul>
      <li class='tc'>
          ...
          <button type="button" onclick="fnPopupmotp();" class="btnPhone mr5">MOTP인증하기</button>
          <button type="button" onclick="fnPopup2();" class="btnPhone mr5">아이핀인증하기</button>
  ...
  <div class="mt10">
    <!--<button onclick="agreeCert(237929,'7Q4VLG',4920)">
      <img src="/m_images/member/btn_ok.png">
    </button>-->
  </div>
</div>
...
<div class="layerArea wid570" style="display:none !important">
	<!-- close -->
	<div class="close"><a href="Javascript:DataResultClose();"><img src="/m_images/common/btn_close01.png" alt="창닫기" /></a></div>
	<!-- title -->
	...
	<div class="infoArea">
		...
		<div class="mycheck">
			<ul>
				<li><a href="Javascript:fnPopup();"><img src="/m_images/sub/support_mycheck_img02.png" alt="휴대폰 인증" /></a></li>
				<!--<li><a href="Javascript:fnPopup2();"><img src="/m_images/sub/support_mycheck_img03.png" class="아이핀(I-PIN) 인증" /></a></li>-->
...
function CertCheckProc(m_trnDT, motp) {
	...
	USRID               = "<?=$LoginMemberID?>";
	COURSE_AGENT_PK     = "<?=$StudyLectureCode?>";
	CLASS_AGENT_PK      = "<?=$StudyLectureCode?>,<?=$LectureTerme_idx?>";
	...
	m_retCD = "000000";
	m_trnID = "<?=$reqseq?>";
    console.log("m_trnID : "+ m_trnID);
	//m_trnDT = "2019-12-01 13:26:31";
	
    if( motp == true ) {

        //인증 처리 성공시 DB작업----------------------------------------------------------------
        $.post("/m_player/play_study_auth_insert.php",
        { 
            'LectureCode'     : LectureCode,
            'StudyLectureCode': StudyLectureCode,
            'Study_Seq'       : Study_Seq,
            ...
            if(data=="Y") {
                ...
                DataResultClose();
                ContentsPlayer2('<?=$LectureCode?>', 1, '<?=$Study_Seq?>', '<?=$StudyLectureCode?>');
            }else{
    ...
    try {
        $.ajax({
            type : "POST",
            url  : "/m_player/play_study_auth_test.php",
            crossDomain: true,
            data : {
                AGTID           : AGTID,
                USRID           : USRID,
