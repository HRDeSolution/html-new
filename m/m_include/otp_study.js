				...
				*/
				...
				if(ConnStatus == "101") {
					if(OtpStatus == "101" || OtpStatus == "103") {
						...
						location.href="/m_player/player_otp_init.php?Chapter_Number="+Chapter_Number+"&LectureCode="+LectureCode+"&Study_Seq="+Study_Seq+"&Chapter_Seq="+Chapter_Seq+"&Contents_idx="+Contents_idx+"&mode="+mode;
					}
					else {
		...
		$.ajax({
			...
			success : function(xml) {					
				if($(xml).find("RetVal").text() == "101") {
					...
					$.post("/m_player/player_captcha_session.php",
					{
						...
						//top.location.reload();
						top.Play(Chapter_Number,LectureCode,Study_Seq,Chapter_Seq,Contents_idx,mode);
					});
...
function ExecuteFDS() {  // 함수명은 자유롭게 설정하셔도 무방합니다.
        var isDebug = true;
        if(isDebug) {
            console.log("ExcuteFDS: " + AGTID + ", " + USRID + ", " + SESSIONID);
        }
        hrdFrame.loadHrdFrame(isDebug);
