...
class Page
{
    ...
    function blockList(){
        ...
        if($this->block != 1){
            ...
            $block_str .= '<div class="circleprev circleprev1"><a href="javascript:readRun2(' . $temp . ');" title="이전 ' . $this->block_size . '" ><i class="ph-bold ph-caret-left"></i></a></div>';
        }else{
            $block_str .= '';
        }
        ...
        for($i = 0; $i < count($arrBlock); $i++){
            if($this->pg != $arrBlock[$i]){
                $block_str .= '<span><a href="javascript:readRun2('. $arrBlock[$i] . ');" >' . $arrBlock[$i] . '</a></span>';
            }else{
                $block_str .= '<span class="on"><a href="javascript:readRun2('. $arrBlock[$i] . ');" >' . $arrBlock[$i] . '</a></span>';
            }
        ...
        if($this->block != $this->block_count && $this->tot_no != 0){
            ...
            $block_str .= '<div class="circlenext circlenext1"><a href="javascript:readRun2(' . $temp . ')" title="다음 ' . $this->block_size . '"><i class="ph-bold ph-caret-right"></i></a></div>';
        }else{
            $block_str .= '';
        }
