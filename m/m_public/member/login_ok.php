...
<?
    ...
    if($Row['Mandatory']=="N") {
	    ...
	    $url = "/m_public/member/terms_first_agree.html?pwchg=$pwchg";

    //비밀번호를 변경해야 하는 경우
	}else if($Row['PassChange']=="N") {
	    $url = "/m_public/member/pwd_first_change.html";
	    $TopLogin="N";

    //사전역량진단을 하지 않은 경우
    }else if ( $Row['AbilityYN'] == "N" ) {
        $url = "/m_archive/prev/prev_test01.html";

    //테스트용 아디이일 경우, 강의페이지로 이동
	} else if ($Row['TestID']=="Y") {
	    $url = "/m_public/mypage/lecture_status.html";

    //그외
	}else{
	    $url = "/m_archive/contents/main.html";
	}
