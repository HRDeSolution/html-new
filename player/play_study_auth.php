...
$EvalCd           = Replace_Check_XSS2($EvalCd);
...
<script language='javascript'>
    ...
    function fnPopupmotp(){
        ...
        document.form_motp.action = "/player/motp.php?class_tme=<?=$Chapter_Number?>&EvalCd=<?=$EvalCd?>&LectureCode=<?=$LectureCode?>&COURSE_AGENT_PK=<?=$StudyLectureCode?>&CLASS_AGENT_PK=<?=$StudyLectureCode?>,<?=$LectureTerme_idx?>&Study_Seq=<?=$Study_Seq?>";
...
function CertCheckProc(m_trnDT, motp) {
    ...
    try {
        $.ajax({
            type : "POST",
            url  : "/player/play_study_auth_test.php",
            crossDomain: true,
            data : {
                AGTID           : AGTID,
                USRID           : USRID,
