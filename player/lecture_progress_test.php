...
$iframeCheckTime = Replace_Check_XSS2($iframeCheckTime);
...
//수강한 전체 차시의 진도율 합
$Sql = "SELECT SUM(IF(Progress>100,100,Progress)) FROM ProgressTest WHERE ID='$LoginMemberID' AND LectureCode='$LectureCode'";
$Result = mysqli_query($connect, $Sql);
$Row = mysqli_fetch_array($Result);
$SUM_Progress = $Row[0];

//수강한 차시수
$Sql = "SELECT COUNT(idx) FROM ProgressTest WHERE ID='$LoginMemberID' AND LectureCode='$LectureCode'";
$Result = mysqli_query($connect, $Sql);
$Row = mysqli_fetch_array($Result);
$ProgressCount = $Row[0];
...
//수강현황 로그 작성
$Sql_log = "INSERT INTO ProgressLogTest(ID, LectureCode, Study_Seq, Chapter_Seq, Contents_idx, ContentsDetail_Seq, LastStudy, Progress, StudyTime, UserIP, RegDate, TriggerYN, Chapter_Number, TotalProgress, FinishTime)
            VALUES('$LoginMemberID', '$LectureCode', $Study_Seq, $Chapter_Seq, $Contents_idx, $ContentsDetail_Seq, '$LastStudy', $ChapterProgress, $ProgressTime, '$UserIP', NOW(), '$TriggerYN', '$Chapter_Number', $Total_Progress, $iframeCheckTime)";
mysqli_query($connect, $Sql_log);

//현재 수강중인 차시가 존재하는지 체크
$Sql = "SELECT * FROM ProgressTest WHERE ID='$LoginMemberID' AND LectureCode='$LectureCode' AND Study_Seq=$Study_Seq AND Chapter_Seq=$Chapter_Seq AND Contents_idx=$Contents_idx";
$Result = mysqli_query($connect, $Sql);
$Row = mysqli_fetch_array($Result);
if($Row) {
    if($ChapterProgress>=100) {
        $Sql2 = "UPDATE ProgressTest SET ContentsDetail_Seq=$ContentsDetail_Seq, LastStudy='$LastStudy', StudyTime=$ProgressTime, UserIP='$UserIP', Progress=$ChapterProgress, TriggerYN='$TriggerYN', Chapter_Number='$Chapter_Number' , FinishTime = $iframeCheckTime
        ...
        $Sql2 = "UPDATE ProgressTest SET ContentsDetail_Seq=$ContentsDetail_Seq, LastStudy='$LastStudy', StudyTime=$ProgressTime, UserIP='$UserIP', RegDate=NOW(), Progress=$ChapterProgress, TriggerYN='$TriggerYN', Chapter_Number='$Chapter_Number' , FinishTime = $iframeCheckTime
                 WHERE ID='$LoginMemberID' AND LectureCode='$LectureCode' AND Study_Seq=$Study_Seq AND Chapter_Seq=$Chapter_Seq AND Contents_idx=$Contents_idx";
    }
} else {
    $Sql2 = "INSERT INTO ProgressTest(ID, LectureCode, Study_Seq, Chapter_Seq, Contents_idx, ContentsDetail_Seq, LastStudy, Progress, StudyTime, UserIP, RegDate, TriggerYN, Chapter_Number, FinishTime)
            VALUES('$LoginMemberID', '$LectureCode', $Study_Seq, $Chapter_Seq, $Contents_idx, $ContentsDetail_Seq, '$LastStudy', $ChapterProgress, $ProgressTime, '$UserIP', NOW(), '$TriggerYN', '$Chapter_Number', $iframeCheckTime)";
}
mysqli_query($connect, $Sql2);

//전체 진도율 계산
$SqlB = "SELECT SUM(StudyTime) FROM ProgressTest WHERE ID = '$LoginMemberID' AND Study_Seq=$Study_Seq";
