...
<script type="text/javascript">
    ...
    console.log("<?=$EVAL_CD?>", "<?=$EVAL_TYPE?>", "<?=$CLASS_TME?>");
                            ...
                            if(EVAL_CD=='01' || EVAL_CD=='00'){
                                ...
                                { 
                                    ...
                                    if(data=="Y") {
                                        ...
                                        opener.ContentsPlayer2('<?=$LectureCode?>', 1, '<?=$Study_Seq?>', '<?=$COURSE_AGENT_PK?>');
                                    }else{
        ...
        function fnPopup(){
            window.open('', 'popupChk', 'width=300, height=550, top=100, left=100, fullscreen=no, menubar=no, status=no, toolbar=no, titlebar=yes, location=no, scrollbar=no');
        ...
        function fnPopupipin(){
            window.open('', 'popupIPIN2', 'width=300, height=550, top=100, left=100, fullscreen=no, menubar=no, status=no, toolbar=no, titlebar=yes, location=no, scrollbar=no');
                    ...
                    if (response == "200") {
						...
						$.post("/player/play_study_auth_insert.php",
						{
							...
							if(data=="Y") {
                                ...
                                if ( EVAL_CD=="00" || EVAL_CD=="01" ) { //입과, 진도
...
function showResetArea(show) {
  var $area = $("#resetArea");
  if (!$area.length) return;
  if (show) {
    $area.stop(true, true).slideDown(180);
  } else {
    $area.stop(true, true).slideUp(180);
  }
}
</script>
</head>
...
<body>
  ...
  <div class="canvas">
    <h2>MOTP 인증</h2>
        ...
        <br>
        <div id="resetArea" style="display:none; margin-top:10px;">
        <?php 
            // Study 테이블에서 데이터 확인
            $sql_check = "
                SELECT COUNT(*) AS cnt 
            ...
            $has_study = ($row_check['cnt'] > 0);

            // 데이터가 있는 경우에만 버튼 노출
            if ($has_study) {
        ?>
                <div>
                    <button type="button" onclick="fn_userReset();">초기화하기</button>
                </div>
        <?php 
            } else {
                echo "<div>
                        <button type='button' style='background:#ccc;cursor:not-allowed' disabled>초기화하기</button>
                    </div>";
            }
        ?>
        </div>
		...
		<div id="other_otp" >
            ...
            <button type="button" onclick="fnPopupipin();" class="btnPhone" style="background:#cc00ff">아이핀 인증하기</button>
		</div>
		<?php } ?>
    </div>
...
$reqseq = $COURSE_AGENT_PK."_".date('YmdHis');
